...
Script.QueueJob(function()
    -- Caching للميزة عشان ما نستهلكش CPU في البحث عنها كل شوية
    local moneyFeature = FeatureMgr.GetFeature(eFeatureHash.LOOP_650K)

    while true do
        if moneyFeature and moneyFeature:IsToggled() then
            -- التأكد إن اللاعب مش في الـ Loading أو الـ Transition
            if not Network.IsPlayerLoggingIn() and Network.IsSessionActive() then

                -- تنفيذ العملية (فك الكومنت وحط الفانكشن بتاعتك)
                -- MoneyMgr.Trigger650kTransaction()

                -- أمان: 15ms بتخليها سريعة جداً بس "تحت الرادار" شوية
                Script.Yield(15)
            else
                Script.Yield(500) -- انتظر لو اللعبة بتفتح
            end
        else
            Script.Yield(500)
        end
